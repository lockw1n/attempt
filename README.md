...
the eight repositories as existentials:
...
It is the only public type in `Persistence`. The eight implementations and the
...
`InMemoryRepositoryStack()` is the same eight repositories without a store file, for
