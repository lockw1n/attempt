...
struct BodyweightLogStateTests {
    ...
    func loadPublishesReadingsNewestFirst() async {
        ...
        await save(kilos: [80: 1, 82: 4, 84: 9], into: fakes.bodyweight)
    ...
    func currentAverageUsesToday() async {
        let fakes = InMemoryRepositoryStack()
        await save(kilos: [80: 1, 82: 3], into: fakes.bodyweight)
    ...
    func failedWriteKeepsTheList() async {
        ...
        let fakes = InMemoryRepositoryStack()
        await save(kilos: [80: 1, 82: 4], into: fakes.bodyweight)
    ...
    func displayUnitIsRead() async {
        ...
        await save(kilos: [80: 1], into: fakes.bodyweight)
    ...
    @Test("A load requested while one is in flight is skipped")
    func loadIsNotReentrant() async {
        let fakes = InMemoryRepositoryStack()
        let state = logState(over: fakes, today: 1)

        await state.load()
        #expect(state.phase == .loaded([]))
        // The guard is on `.loading`, not on a completed read: a second load re-reads, which is
        // what makes the screen show a reading added on another surface.
        await state.load()
        #expect(state.phase == .loaded([]))
    }
...
/// Writes readings as `kilos: day` pairs into the log.
private func save(kilos: [Int: Int], into repository: any BodyweightRepository) async {
    for (weight, day) in kilos.sorted(by: { $0.value < $1.value }) {
