...
/// What the suite above does to a workout, in one place: start it, log into it, adjust a set, and
/// read the deviation back off the card.
///
...
extension ActiveSessionStore {
    ...
    fileprivate func logFirstSquatSet(grams: Int, reps: Int) async throws -> SetEntry {
    ...
    fileprivate func comparisonForFirstSet() -> PlannedTargetComparison? {
...
extension RoutineFixture {
    ...
    fileprivate func startedWorkout() async throws -> ActiveSessionStore {
    ...
    fileprivate func template() async throws -> [RoutineTargetGroup] {
    ...
    fileprivate func start(_ store: ActiveSessionStore) async throws {
