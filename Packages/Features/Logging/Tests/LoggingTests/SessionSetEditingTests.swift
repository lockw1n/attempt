...
struct SessionSetEditingTests {
    ...
    func editingCarriesEveryOtherFieldAcross() async throws {
        ...
        // Empty on both sides by construction — neither the logged set nor the edit picked any.
        // The comparison that can actually fail is in the past-session suite, whose fixture writes a
        // set carrying two.
...
struct PastSessionSetEditingTests {
    ...
    func editingAPastSetLeavesEverySessionAlone() async throws {
        ...
        let wrote = try await history.writer.edit(
            ...
            // The modifiers are what the editor opened holding — the fixture's own two, one of them
            // unrecognised. `FR-1.2.8` collects the column now, so an edit that did not hand it
            // back would delete it.
            to: SetEntryValues(
        ...
        // `FR-1.2.8`'s round trip: a spelling the app does not recognise went into the form, came
        // back out of it and is still on the row.
