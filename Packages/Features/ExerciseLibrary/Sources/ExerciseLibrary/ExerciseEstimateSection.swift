...
///
/// **Four states over seven sentences.** The insufficient-data case carries `FR-1.7.1`'s reason
...
enum ExerciseEstimateScreenState: Equatable {
    ...
    /// There is a number, under the formula and window it was produced with.
    case ready(DatedRecord, formula: E1RMFormulaID, days: Int)
    ...
    static func current(_ state: ExerciseRecordsState) -> Self {
        ...
        case .record(let record):
            return .ready(record, formula: estimate.formula, days: estimate.lookback.days)
...
struct ExerciseEstimateReading: View {
    ...
    /// The heading, then whichever of the four states is current.
    var body: some View {
        GroupedSection(Text(ExerciseLibraryStrings.e1rmSection)) {
            switch state {
            case .loading:
                LoadingStateView()
            case .ready(let record, let formula, let days):
                estimate(record, formula: formula, days: days)
    ...
    /// The number, with what produced it under it.
    ///
    ...
    private func estimate(
        _ record: DatedRecord, formula: E1RMFormulaID, days: Int
    ) -> some View {
        MetricTile(
