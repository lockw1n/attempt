...
public struct ExerciseListView: View {
    ...
    ///
    /// **A ``DesignSystem/SearchField`` in the content, not `.searchable`** (`FR-16.5.4`). T-1.10
    /// chose the modifier for being the system's own affordance, and the answer to that is what the
    ...
    public var body: some View {
        ...
        return ScrollView {
            VStack(alignment: .leading, spacing: Spacing.lg.points) {
                SearchField(
                    text: $state.searchText, prompt: ExerciseLibraryStrings.searchPrompt)
