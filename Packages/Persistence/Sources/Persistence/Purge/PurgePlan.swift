...
struct PurgePlan {
    ...
    init(context: ModelContext, scope: PurgeScope) throws {
        self.scope = scope
        exercises = try context.allRows(ExerciseEntity.self, includingDeleted: true)
        sessions = try context.allRows(WorkoutSessionEntity.self, includingDeleted: true)
        entries = try context.allRows(ExerciseEntryEntity.self, includingDeleted: true)
        sets = try context.allRows(SetEntryEntity.self, includingDeleted: true)
        trainingMaxes = try context.allRows(TrainingMaxConfigEntity.self, includingDeleted: true)
        trainingMaxHistory = try context.allRows(
            TrainingMaxHistoryEntity.self, includingDeleted: true)
        bodyweight = try context.allRows(BodyweightEntryEntity.self, includingDeleted: true)
        equipment = try context.allRows(EquipmentProfileEntity.self, includingDeleted: true)
        settings = try context.allRows(UserSettingsEntity.self, includingDeleted: true)
        records = try context.allRows(PersonalRecordCacheEntity.self, includingDeleted: true)
        routines = try context.allRows(RoutineEntity.self, includingDeleted: true)
        routineExercises = try context.allRows(RoutineExerciseEntity.self, includingDeleted: true)
        targetGroups = try context.allRows(RoutineTargetGroupEntity.self, includingDeleted: true)
        plannedTargets = try context.allRows(PlannedTargetGroupEntity.self, includingDeleted: true)
        programs = try context.allRows(ProgramEntity.self, includingDeleted: true)
        programDays = try context.allRows(ProgramDayEntity.self, includingDeleted: true)
        programRuns = try context.allRows(ProgramRunEntity.self, includingDeleted: true)
