...
extension PlannedTargetGroupEntity {
    static func matchingID(_ id: UUID) -> Predicate<PlannedTargetGroupEntity> {
        #Predicate<PlannedTargetGroupEntity> { $0.id == id }
    }

    static func matchingIDs(_ ids: Set<UUID>) -> Predicate<PlannedTargetGroupEntity> {
        #Predicate<PlannedTargetGroupEntity> { ids.contains($0.id) }
    }

    static var notDeleted: Predicate<PlannedTargetGroupEntity> {
        #Predicate<PlannedTargetGroupEntity> { $0.deletedAt == nil }
    }

    static func notDeleted(
        alsoMatching other: Predicate<PlannedTargetGroupEntity>
    ) -> Predicate<PlannedTargetGroupEntity> {
        #Predicate<PlannedTargetGroupEntity> { entity in
            other.evaluate(entity) && entity.deletedAt == nil
        }
    }

    static func softDeleted(onOrBefore cutoff: Date) -> Predicate<PlannedTargetGroupEntity> {
        #Predicate<PlannedTargetGroupEntity> { entity in
