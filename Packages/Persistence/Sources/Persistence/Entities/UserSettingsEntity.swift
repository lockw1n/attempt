...
extension UserSettingsEntity {
    static func matchingID(_ id: UUID) -> Predicate<UserSettingsEntity> {
        #Predicate<UserSettingsEntity> { $0.id == id }
    }

    static func matchingIDs(_ ids: Set<UUID>) -> Predicate<UserSettingsEntity> {
        #Predicate<UserSettingsEntity> { ids.contains($0.id) }
    }

    static var notDeleted: Predicate<UserSettingsEntity> {
        #Predicate<UserSettingsEntity> { $0.deletedAt == nil }
    }

    static func notDeleted(
        alsoMatching other: Predicate<UserSettingsEntity>
    ) -> Predicate<UserSettingsEntity> {
        #Predicate<UserSettingsEntity> { entity in
            other.evaluate(entity) && entity.deletedAt == nil
        }
    }

    static func softDeleted(onOrBefore cutoff: Date) -> Predicate<UserSettingsEntity> {
        #Predicate<UserSettingsEntity> { entity in
