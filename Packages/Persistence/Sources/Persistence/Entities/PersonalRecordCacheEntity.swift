...
extension PersonalRecordCacheEntity {
    static func matchingID(_ id: UUID) -> Predicate<PersonalRecordCacheEntity> {
        #Predicate<PersonalRecordCacheEntity> { $0.id == id }
    }

    static func matchingIDs(_ ids: Set<UUID>) -> Predicate<PersonalRecordCacheEntity> {
        #Predicate<PersonalRecordCacheEntity> { ids.contains($0.id) }
    }

    static var notDeleted: Predicate<PersonalRecordCacheEntity> {
        #Predicate<PersonalRecordCacheEntity> { $0.deletedAt == nil }
    }

    static func notDeleted(
        alsoMatching other: Predicate<PersonalRecordCacheEntity>
    ) -> Predicate<PersonalRecordCacheEntity> {
        #Predicate<PersonalRecordCacheEntity> { entity in
            other.evaluate(entity) && entity.deletedAt == nil
        }
    }

    static func softDeleted(onOrBefore cutoff: Date) -> Predicate<PersonalRecordCacheEntity> {
        #Predicate<PersonalRecordCacheEntity> { entity in
