...
extension TrainingMaxConfigEntity {
    ...
    static var notDeleted: Predicate<TrainingMaxConfigEntity> {
        #Predicate<TrainingMaxConfigEntity> { $0.deletedAt == nil }
    }

    static func notDeleted(
        alsoMatching other: Predicate<TrainingMaxConfigEntity>
    ) -> Predicate<TrainingMaxConfigEntity> {
        #Predicate<TrainingMaxConfigEntity> { entity in
            other.evaluate(entity) && entity.deletedAt == nil
        }
    }

    static func softDeleted(onOrBefore cutoff: Date) -> Predicate<TrainingMaxConfigEntity> {
        #Predicate<TrainingMaxConfigEntity> { entity in
