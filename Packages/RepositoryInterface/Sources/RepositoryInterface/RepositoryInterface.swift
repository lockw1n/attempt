...
//
//    FIVE READS DO NOT TAKE IT, and each says so on itself rather than relying on this list.
//    `SettingsRepository.settings()`, because a soft-deleted settings row is a withdrawn identity
//    and no requirement describes one. `TrainingMaxRepository`'s two in-force lookups,
//    `EquipmentRepository.defaultProfile()` and `ProgramRepository.currentRun()`, because they
//    *resolve* to the one row in force rather than enumerating: a deleted configuration is a
//    configuration the user replaced, a deleted training max is a number they withdrew, a deleted
//    profile is a gym they left, and a deleted run is one whose program they deleted, so including
//    any of them would answer a question nobody asked with a row the store keeps only for a purge.
//    Their history-shaped siblings — `configurationHistory`, `history`, `profiles` and `runs` — do
//    take the flag, which is where an export or a diagnostic goes.
//
