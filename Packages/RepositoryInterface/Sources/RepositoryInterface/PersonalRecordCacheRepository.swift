...
/// identified by the `(exercise, repCount, setCount)` triple the call already names, so an `id` here would be a
...
public struct PersonalRecordCacheValues: Sendable, Hashable {
    ...
    /// How many consecutive sets the scheme asks for (`FR-16.2.1`). One is `FR-1.6.1`'s rep max.
    public let setCount: Int

    /// The record weight. Signed: assisted work records a negative load.
    public let weight: Weight

    /// The `SetEntry` holding the record — the run's first set. See ``PersonalRecordCache/sourceSetID``.
    public let sourceSetID: UUID

    /// When the record was set, taken from the source set's session.
    public let achievedAt: Date

    /// The load this record beat at this scheme, or `nil` for a baseline (`FR-16.2.3`).
    ...
    /// The cell these values are the record for — the row's identity, with the exercise.
    public var scheme: RecordScheme { RecordScheme(reps: repCount, sets: setCount) }

    /// Creates the values one cached row is written from.
    ///
    /// ``setCount`` and ``previousWeight`` default to the rep max's own values, for
    /// ``PersonalRecordCache/init(id:createdAt:updatedAt:deletedAt:exerciseID:repCount:setCount:weight:sourceSetID:achievedAt:previousWeight:computationVersion:)``'s
    /// reason.
    public init(
        repCount: Int,
        setCount: Int = 1,
...
public protocol PersonalRecordCacheRepository: Sendable {
    /// One exercise's cached scheme records, ascending by
    /// ``PersonalRecordCache/repCount`` and then ``PersonalRecordCache/setCount``.
    ///
    ...
    ///
    /// **No exercise, and no limit either.** The table holds at most sixty rows per exercise, so the
    ...
    ///
    /// **Reconciled per scheme, not cleared and rewritten.** A row's identity is its
    /// `(exerciseID, repCount, setCount)` triple, so a cell still holding a record keeps the row it
    /// had — which is what lets `FR-1.6.2`'s link to a record survive a recompute that did not move
    /// it. A cell in `values` with no row yet gets one; a cell with a row and no entry in `values`
    /// is **soft-deleted** (`G-1.3`), because a record that no longer stands is not a record and
    /// hard deletion happens only in a purge.
    ///
    ...
    ///   - values: Every record the exercise now holds. At most one entry per
    ///     ``PersonalRecordCacheValues/scheme``; a repeated one resolves to whichever came last,
    ///     since the triple is the identity.
