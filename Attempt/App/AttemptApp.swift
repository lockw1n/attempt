...
///
/// The wiring it does today is `TR-1.1`'s restoration loop: read the stored position before the
/// first frame, and write it back whenever it changes. Both halves belong here rather than in
/// `RootTabView`, because a view that persisted its own navigation would persist it once per place
/// it was used.
@main
struct AttemptApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            RootTabView(navigation: navigation)
